...
disallowLogout = true			-- Set whether to show the userpanel to the players if they log out of their accounts.
...
infoText = "An account is needed so script can work and save data. Registration is fast!\n\n------ Rules ------ \n\n* No cheating!\n* No insulting and spamming! Be polite!\n* English or italian languages are preferred.\n* Remember that's a home-run test server\n* HAVE FUN!\n\nThis server has Dynamic IP, use the filter to find it."
...
function loginPanel()
		...
		if xmlFile then
			...
			if (status == "true") then
				...
				btnToggleRegister = guiCreateButton(548,368,168,74,"Register an account",false,mainWindow)
...
function hideLoginWindow()
	guiSetInputEnabled(false)
	if mainWindow then guiSetVisible(mainWindow, false) end
	if registerWindow then guiSetVisible(registerWindow, false) end
	showCursor(false)
	if blackLoginScreen == true then
		fadeCamera(true,removeBlackScreenTime)
	end
